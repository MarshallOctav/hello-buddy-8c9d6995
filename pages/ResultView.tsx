  ...
  return (
    <div className="mx-auto max-w-5xl px-3 py-4 sm:px-6 sm:py-8 lg:px-8">
      <PageHeader 
        ...
        subtitle={
           <span className="flex flex-wrap items-center gap-2 sm:gap-3">
              <span className="flex items-center gap-1.5 bg-white/10 px-2 sm:px-3 py-1 rounded-full text-xs font-medium text-slate-300 border border-white/10">
                 <Target className="h-3 w-3" /> {test.category}
              </span>
              <span className="text-xs sm:text-sm">{content.result.date} {new Date().toLocaleDateString()}</span>
           </span>
        }
        badge={
             <Link to="/catalog" className="inline-flex items-center text-xs sm:text-sm font-medium text-slate-400 hover:text-white transition-colors">
                <ArrowLeft className="mr-1.5 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4" /> Back to Catalog
             </Link>
        }
      >
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
            <Button variant="secondary" onClick={() => setIsShareModalOpen(true)} className="bg-white/20 text-white hover:bg-white/30 border border-white/30 text-sm sm:text-base px-3 sm:px-4 py-2 sm:py-2.5">
                <Share2 className="mr-1.5 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4" /> {content.result.share}
            </Button>
            <Button variant="primary" onClick={handleDownloadPDF} className="bg-white text-slate-900 hover:bg-slate-100 shadow-lg text-sm sm:text-base px-3 sm:px-4 py-2 sm:py-2.5">
                <Download className="mr-1.5 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4" /> {content.result.download}
                {!canAccess(userPlan, AccessLevel.PRO) && (
                  <Star className="ml-1 sm:ml-1.5 h-3 w-3 sm:h-3.5 sm:w-3.5 text-blue-500" />
                )}
            </Button>
        </div>
      </PageHeader>
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:gap-8 lg:grid-cols-3">
        {/* Score Card */}
        <div className="rounded-xl sm:rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 lg:p-8 text-center shadow-sm lg:col-span-1">
          <h3 className="text-base sm:text-lg font-semibold text-slate-500">{content.result.score}</h3>
          <div className="my-4 sm:my-6 flex justify-center">
            <div className="relative flex h-28 w-28 sm:h-36 sm:w-36 lg:h-40 lg:w-40 items-center justify-center rounded-full border-6 sm:border-8 border-slate-100">
               ...
               <span className="text-3xl sm:text-4xl font-bold text-slate-900">{score}</span>
            </div>
          </div>
          <div className={`inline-block rounded-full px-3 sm:px-4 py-1 text-xs sm:text-sm font-bold ${bg} ${color}`}>
            {level}
          </div>
        </div>

        {/* Visual Analysis - Dynamic Chart */}
        <div className="min-h-[280px] sm:min-h-[300px] rounded-xl sm:rounded-2xl border border-slate-200 bg-white p-3 sm:p-4 shadow-sm lg:col-span-2">
            <h3 className="mb-3 sm:mb-4 text-center text-sm sm:text-base font-semibold text-slate-500">{content.result.breakdown}</h3>
              ...
              {isFinancialHealthTest ? (
                ...
                <div className="flex flex-col items-center justify-around h-full gap-2">
                  ...
                  <div className="w-full px-2 sm:px-4">
                    <h4 className="font-bold text-slate-700 text-xs sm:text-sm mb-2 text-center">{language === 'id' ? 'Perbandingan dengan Ideal' : 'Comparison with Ideal'}</h4>
                    <div className="grid grid-cols-3 gap-2 sm:gap-4 text-xs sm:text-sm">
                      {pieData.map((item, i) => (
                        <div key={i} className="flex flex-col items-center gap-1 p-2 rounded-lg bg-slate-50">
                          <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full" style={{ backgroundColor: getPieColor(item, i) }}></div>
                          <span className="font-medium text-slate-600 text-[10px] sm:text-xs">{item.name}</span>
                          <span className={`text-xs sm:text-sm font-bold ${item.value > item.ideal && i < 2 ? 'text-red-600' : item.value < item.ideal && i === 2 ? 'text-red-600' : 'text-green-600'}`}>
                            {item.value}%
                          </span>
                          <span className="text-[10px] sm:text-xs text-slate-400">ideal: {item.ideal}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      {isFinancialHealthTest && financialHealthData?.insights?.length > 0 && (
        <div className="mt-4 sm:mt-6 lg:mt-8 rounded-xl sm:rounded-2xl border border-amber-200 bg-amber-50 p-4 sm:p-6 shadow-sm">
          <h3 className="flex items-center gap-2 text-base sm:text-lg font-bold text-amber-800 mb-3 sm:mb-4">
          ...
          <ul className="space-y-2 sm:space-y-3">
            {financialHealthData.insights.map((insight: string, i: number) => (
              <li key={i} className="flex items-start gap-2 sm:gap-3 text-sm sm:text-base text-amber-900/80">
                <span className="mt-0.5 flex h-4 w-4 sm:h-5 sm:w-5 flex-none items-center justify-center rounded-full bg-amber-200 text-amber-800 text-[10px] sm:text-xs font-bold">{i + 1}</span>
                <span>{insight}</span>
              </li>
            ))}
          </ul>
      ...
      <div className="mt-4 sm:mt-6 lg:mt-8 grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
         <div className="rounded-xl sm:rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm flex flex-col">
            <h3 className="flex items-center gap-2 text-base sm:text-lg font-bold text-slate-900 mb-3 sm:mb-4">
         ...
         <div className="rounded-xl sm:rounded-2xl border border-orange-200 bg-orange-50 p-4 sm:p-6 shadow-sm relative overflow-hidden flex flex-col justify-center">
            <div className="absolute top-0 right-0 -mt-8 -mr-8 h-24 sm:h-32 w-24 sm:w-32 rounded-full bg-orange-200/50 blur-2xl"></div>
             <h3 className="flex items-center gap-2 text-base sm:text-lg font-bold text-orange-800 mb-3 sm:mb-4 relative z-10">
                 <Target className="h-4 w-4 sm:h-5 sm:w-5 text-orange-600" />
                 {language === 'id' ? 'Poin Vital' : 'Vital Point'}
            </h3>
            <div className="relative z-10">
                <p className="text-orange-900/80 text-sm sm:text-base lg:text-lg font-bold leading-tight">
                    {getVitalPoint(score)}
                </p>
                <div className="mt-3 sm:mt-4 flex items-center gap-2 text-[10px] sm:text-xs font-bold text-orange-700 uppercase tracking-wider">
      ...
      {/* Methodology Section */}
      <div className="mt-4 sm:mt-6 lg:mt-8 rounded-xl sm:rounded-2xl border border-blue-100 bg-blue-50 p-4 sm:p-6 lg:p-8">
        <div className="flex items-start gap-3 sm:gap-4">
          <div className="hidden rounded-full bg-blue-100 p-2 sm:p-3 text-blue-600 sm:block">
            <Library className="h-5 w-5 sm:h-6 sm:w-6" />
          </div>
          <div className="flex-1">
             <h2 className="text-lg sm:text-xl font-bold text-slate-900">{content.result.methodology.title}</h2>
             <div className="mt-3 sm:mt-4 grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
                <div>
                   <h4 className="flex items-center gap-2 text-xs sm:text-sm font-bold text-slate-500">
                     <BookOpen className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> {content.result.methodology.source}
                   </h4>
                   <p className="mt-1 text-sm sm:text-base font-medium text-slate-900">{test.methodology.name}</p>
                   <p className="text-xs sm:text-sm italic text-slate-600">{test.methodology.source}</p>
                </div>
                <div>
                   <h4 className="flex items-center gap-2 text-xs sm:text-sm font-bold text-slate-500">
                     <UserIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> {content.result.methodology.author}
                   </h4>
                   <p className="mt-1 text-sm sm:text-base font-medium text-slate-900">{test.methodology.author}</p>
                </div>
             </div>
             <div className="mt-3 sm:mt-4 border-t border-blue-200 pt-3 sm:pt-4">
               <h4 className="text-xs sm:text-sm font-bold text-slate-500">{content.result.methodology.why}</h4>
               <p className="mt-1 text-xs sm:text-sm leading-relaxed text-slate-700">{test.methodology.description}</p>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 sm:mt-6 lg:mt-8 rounded-xl sm:rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 lg:p-8 shadow-sm">
        <h2 className="mb-4 sm:mb-6 text-lg sm:text-xl font-bold text-slate-900">{content.result.recommendations}</h2>
        <div className="space-y-3 sm:space-y-4">
            {recommendations.map((rec, i) => (
              <div key={i} className="flex gap-3 sm:gap-4">
                  <CheckCircle2 className="mt-0.5 h-5 w-5 sm:h-6 sm:w-6 flex-none text-green-500" />
                  <div>
                      <h4 className="text-sm sm:text-base font-semibold text-slate-900">{rec.title}</h4>
                      <p className="text-xs sm:text-sm text-slate-600">{rec.desc}</p>
                  </div>
              </div>
            ))}
        </div>
      ...
      <div className="mt-6 sm:mt-8 text-center">
        <Link to="/catalog">
            <Button variant="secondary" className="text-sm sm:text-base px-4 sm:px-6">
                <RotateCcw className="mr-1.5 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4" /> {content.result.retake}
            </Button>
        </Link>
      </div>
